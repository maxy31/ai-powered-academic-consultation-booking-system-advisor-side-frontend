...
const EditAnnouncementScreen = () => {
  ...
  const { announcement } = route.params as any;

  const [title, setTitle] = useState(announcement.title);
  const [content, setContent] = useState(announcement.content);
  const [publisherName, setPublisherName] = useState(announcement.publisherName);
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const res = await fetch('http://10.0.2.2:8080/api/announcements/updateAnnouncement', {
        method: 'POST',
        headers: {
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.outerContainer}>
        <View style={styles.formCard}>
          <Text style={styles.formTitle}>Edit Your Announcement</Text>
          <TextInput
            style={styles.input}
            value={title}
